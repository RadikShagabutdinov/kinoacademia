...
const directionFor = (tx: RatingTransactionDto, companyId: string): 'in' | 'out' | 'neutral' => {
  if (tx.recipientCompanyId === companyId) return 'in';
  if (tx.donorCompanyId === companyId) return 'out';
  return 'neutral';
};

export const CompanyHistoryList = ({ transactions, companyId }: Props) => {
  ...
  return (
    <section>
      ...
      {transactions.length === 0 ? (
        <NoticeBox tone="muted">Транзакций пока нет.</NoticeBox>
      ) : (
        <div
          ref={parentRef}
        ...
        >
          <ul className="relative w-full" style={{ height: `${virtualizer.getTotalSize()}px` }}>
            {virtualizer.getVirtualItems().map((vi) => {
              ...
              if (!tx) return null;
              const dir = directionFor(tx, companyId);
