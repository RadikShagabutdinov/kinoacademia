...
/** Постоянный рейтинг победителю номинации (для «Лучшего фильма» — режиссёру). */
export const OSCAR_WIN_PERSON_BONUS = 400;
/**
 * Постоянный рейтинг компании, номинировавшей победителя, — начисляется отдельно и
 * только если у победителя есть действующий контракт (любой, хоть временный) с ней.
 */
export const OSCAR_WIN_COMPANY_BONUS = 400;
/**
 * Утешительный приз: постоянный рейтинг каждому номинанту, не получившему статуэтку.
 * Присуждение закрывает номинацию, поэтому выплачивается сразу всем остальным.
 */
export const OSCAR_NOMINEE_BONUS = 100;
